...
### 許可される差異: データI/Oレイヤーのみ

```
バックテスト: CSV → [共通トレードロジック] → CSV/メトリクス出力
リアルトレード: MT5 → [共通トレードロジック] → MT5注文実行
```

- **バックテスト固有**: データ読み込み（`DataLoader`）、シミュレーション実行（`TradeSimulator`）、
  メトリクス集計（`MetricsCalculator`）、結果出力
- **リアル固有**: MT5接続（`MT5ConnectionManager`）、MT5データ取得（`MT5DataProvider`）、
  MT5注文実行（`MT5TradeExecutor`）、ティックエントリ最適化
- **共通（変更禁止）**: `decision/`（シグナル生成）、`calculator/`（指標計算）、
  `constraint/`（ガード条件）、`core/`（エンティティ・インターフェース）

### バックテストモジュールに書いてよいもの

- データ読み込み・書き出しのインターフェース
- CSV↔DataFrame変換
