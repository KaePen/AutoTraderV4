...
def run_one(max_positions: int, start_year: int, end_year: int) -> CompareRow:
...
def save_report(
    ...
    out_path = Path("reports") / f"max_positions_comparison_{today}.md"
...
def main() -> None:
    ...
    for max_pos in max_pos_list:
        row = run_one(max_pos, start_year, end_year)
        rows.append(row)
