...
class TradeSimulator:
    ...
    def _partial_close_position(
        ...
        # 損益計算（按分）
        if position.signal_type == SignalType.BUY:
            profit_pips = (exit_price - position.entry_price) / self._pip_unit
        else:
            profit_pips = (position.entry_price - exit_price) / self._pip_unit
    ...
    def _update_equity(self, candle: Candle) -> None:
        ...
        for position in open_pos:
            if position.signal_type == SignalType.BUY:
                pips = (close_price - position.entry_price) / self._pip_unit
            else:
                pips = (position.entry_price - close_price) / self._pip_unit
    ...
    def _update_mfe_mae(self, candle: Candle) -> None:
        ...
        for position in self.state.open_positions:
            ...
            if position.signal_type == SignalType.BUY:
                fav = (candle.high - position.entry_price) / self._pip_unit
                adv = (candle.low - position.entry_price) / self._pip_unit
            else:
                fav = (position.entry_price - candle.low) / self._pip_unit
                adv = (position.entry_price - candle.high) / self._pip_unit
